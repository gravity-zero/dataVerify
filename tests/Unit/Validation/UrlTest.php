...
class UrlTest extends TestCase
{
    ...
    public function testUrlRejectsEmptyHost(): void
    {
        $verifier = new DataVerify((object)['url' => 'http://']);
        $verifier->field('url')->url();
        $this->assertFalse($verifier->verify());
    }

    public function testUrlAcceptsIpv4WithRequireTld(): void
    {
        $verifier = new DataVerify((object)['url' => 'http://192.168.1.1']);
        $verifier->field('url')->url(requireTld: true);
        $this->assertTrue($verifier->verify());
        
        $verifier2 = new DataVerify((object)['url' => 'http://8.8.8.8:8080/path']);
        $verifier2->field('url')->url(requireTld: true);
        $this->assertTrue($verifier2->verify());
    }

    public function testUrlRejectsValueWithoutScheme(): void
    {
        $verifier = new DataVerify((object)['url' => 'example.com']);
        $verifier->field('url')->required->url();
        $this->assertFalse($verifier->verify());
    }

    public function testUrlRejectsInvalidUrlStructure(): void
    {
        $verifier = new DataVerify((object)['url' => 'http://']);
        $verifier->field('url')->required->url();
        $this->assertFalse($verifier->verify());
    ...
    public function testUrlRejectsNonStringHost(): void
    {
        $verifier = new DataVerify((object)['url' => 'http://[:]']);
        $verifier->field('url')->required->url();
        $this->assertFalse($verifier->verify());
    ...
    public function testUrlAcceptsIpv4WhenTldRequired(): void
    {
        $verifier = new DataVerify((object)['url' => 'http://192.168.1.1']);
        $verifier->field('url')->required->url(requireTld: true);
        $this->assertTrue($verifier->verify());
    }
}
